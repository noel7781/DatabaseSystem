...
int find_path(const char* pathname) {
    int i = 1;
...
int db_find(int table_id, int64_t key, char * ret_val, int trx_id) {
    ...
    while(SUCCESS) {
        if(txnMgr.trx_table[trx_id]->is_abort == true) {
            return -1;
        }
    ...
    while(SUCCESS) {
        ...
        if(pthread_mutex_trylock(&target->buffer_page_latch) != 0) {
            ...
            if(txnMgr.trx_table[trx_id]->state == RUNNING) {
                ...
                if(tmp != NULL) {
                    UndoLog saveLog;
                    saveLog.table_id = table_id;
                    saveLog.key = key;
                    strcpy(saveLog.old_value, tmp->value);
                    txnMgr.trx_table[trx_id]->undo_log_list.push_back(saveLog);
