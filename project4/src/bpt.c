        ...
        while(1) {
            for(int i = 0; i < cur->frame.number_of_keys; i++) {
                ...
                if(right == NULL) continue;
                    ...
                    int j = 0;
                    /*
                    for(j = 0; j < right->frame.number_of_keys; j++) {
                        if(right->frame.record[j].key == cur->frame.record[i].key) break;
                    }
                    if(j != right->frame.number_of_keys) {
...
int binary_search(record *arr, int size, int target) {
    int start_idx = 0;
    int end_idx = size - 1;
    int mid_idx = 0;
...
char str_list[MAX_LENGTH+1];
for(int i = 0; i < MAX_COUNT; i++) {
    for(int j = 0; j < MAX_LENGTH; j++) {
        str_list[j] = 'a' + rand() % 26;
    }
    db_insert(tableId, key++, str_list);
}
*/
